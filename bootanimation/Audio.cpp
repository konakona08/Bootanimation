...
bool Audio::Convert(void)
{
	...
	pAudioBuffer = (int16_t*)malloc((llFrameCount * OUT_SAMPLE_RATE / stDecoder.outputSampleRate) * nChannels * sizeof(int16_t) + (1024 * 1024));//add 1024k as boundary pad
	memset(pAudioBuffer, 0, (llFrameCount * OUT_SAMPLE_RATE / stDecoder.outputSampleRate) * nChannels * sizeof(int16_t) + (1024 * 1024));
